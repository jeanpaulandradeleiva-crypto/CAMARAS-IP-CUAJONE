...
- `ppe-fall` and `ppe-only` delegate tracking and analytics to the native pipeline;
  `ppe-only` does not load pose.
- Events and evidence are persisted locally. The repository does not currently
  publish events or metadata to Milestone. The authoritative MSI output follows
  the shared [operator CSV/evidence contract v1](operator-evidence-contract-v1.md):
  `Reporte_Eventos_Seguridad.csv` plus `Evidencias/`. XLSX is local/offline Python
  QA only and is not generated by the installed runtime.
...
Recommendation: AI Bridge is a future native analytics integration candidate and
